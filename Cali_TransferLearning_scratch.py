...
mcp_save = ModelCheckpoint('best_s2y_scratch35rasp2.hdf5', save_best_only=True, monitor='val_loss', mode='min')
import time
start = time.time()
...
#%%
model = models.load_model('best_s2y_scratch35rasp2.hdf5')
pred = model.predict(hists_val).flatten()
#np.savetxt('sadok_7AGM_scratch.txt', pred)
...
plt.title('Trained from Scratch on Raspberry Data')
...
#plt.plot(yields_test, label='True Values');
...
pred = []
for j in range(n):
    model = models.load_model('TL Models/best_s2y_scratch35rasp'+str(j)+'.hdf5')
...
for j in range(n):
    ...
    pred.append(model.predict(hists_val).flatten())
avg_preds = np.mean(pred,axis=0)
np.savetxt('Raspberry_yield_scratch.txt', avg_preds)
# %%
